...
import { externalCampaignSchema, validateExternalCampaignInput } from "../../_service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const validation = await validateExternalCampaignInput(parsed.data);
    await writeAuditLog(null, "external_api.campaign_dry_run", "campaign", {
      endpoint: "/api/external/v1/campaigns/dry-run",
    ...
    return NextResponse.json(
      {
        ...
        selectedSmtpCount: validation.selectedSmtpCount,
        estimatedThroughput: validation.estimatedThroughput,
